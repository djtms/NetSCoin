...
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // netscoin: starting difficulty is 1 / 2^12
...
static const int64 nTargetTimespan = 30 * 60;						// netscoin: difficulty re-target every 30 blocks/minute
static const int64 nTargetSpacing = 60;								// netscoin: 1 minute blocks
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    // fMiner is true when called from the internal netscoin miner
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "netscoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
CBlock* CreateNewBlock(CReserveKey& reservekey)
{
    ...
    int64 nFees = 0;
    {
        ...
        while (!mapPriority.empty())
        {
            ...
            // netscoind: Reduce the exempted free transactions to 500 bytes (from Bitcoin's 3000 bytes)
